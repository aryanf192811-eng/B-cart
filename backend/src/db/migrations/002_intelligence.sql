...
SELECT
  ...
  COALESCE(ROUND(AVG(
    CASE WHEN po.status = 'fully_received' AND po.received_at::date <= po.expected_delivery_date THEN 100.0
         WHEN po.status = 'fully_received' THEN 0.0
         ELSE NULL END
  )::numeric, 1), 100) AS on_time_rate,

  COALESCE(ROUND((
    SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN pol.qty_received ELSE 0 END) * 100.0 / 
    NULLIF(SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN pol.qty_ordered ELSE 0 END), 0)
  )::numeric, 1), 100) AS fulfillment_rate,

  COALESCE(ROUND((
    100.0 - SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN COALESCE(pol.rejected_qty, 0) ELSE 0 END) * 100.0 / 
    NULLIF(SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN pol.qty_received ELSE 0 END), 0)
  )::numeric, 1), 100) AS quality_rate,

  COALESCE(ROUND((
    COALESCE(AVG(CASE WHEN po.status = 'fully_received' AND po.received_at::date <= po.expected_delivery_date THEN 100.0 WHEN po.status = 'fully_received' THEN 0.0 ELSE NULL END), 100) * 0.5
    + COALESCE((SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN pol.qty_received ELSE 0 END) * 100.0 / NULLIF(SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN pol.qty_ordered ELSE 0 END), 0)), 100) * 0.3
    + COALESCE((100.0 - SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN COALESCE(pol.rejected_qty, 0) ELSE 0 END) * 100.0 / NULLIF(SUM(CASE WHEN po.status IN ('fully_received', 'partially_received') THEN pol.qty_received ELSE 0 END), 0)), 100) * 0.2
